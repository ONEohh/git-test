...
import stat
import time
import yaml  # 新增：用于解析 YAML Front Matter
...
def list_available_skills(base_dir="agents/skills"):
    """
    扫描 base_dir 下的所有一级子目录，读取每个 SKILL.md 的 YAML front matter，
    提取 name 和 description 字段，返回列表。
    """
    skills = []
    ...
    for folder in os.listdir(base_dir):
        ...
        # 提取 YAML front matter (第一个 --- 和第二个 --- 之间的内容)
        if content.startswith("---"):
            parts = content.split("---", 2)  # 最多分割2次，得到 ['', yaml_str, markdown_body]
            if len(parts) >= 3:
                yaml_str = parts[1]
                try:
...
def load_skill_content(skill_name: str, base_dir="agents/skills"):
    """
    加载指定技能 SKILL.md 文件的正文（YAML front matter 之后的部分）。
    返回字符串内容，如果未找到则返回提示信息。
    """
    skill_md_path = os.path.join(base_dir, skill_name, "SKILL.md")
    if not os.path.isfile(skill_md_path):
        return f"错误：未找到技能 '{skill_name}' 的 SKILL.md 文件。"
...
def execute_tool_call(tool_call):
    ...
    elif tool_name == "query_anythingllm":
        question = tool_args.get('question')
        res = anythingllm_query(question)
        if res.get("success"):
            result = json.dumps({"status": "success", "data": res.get("content", "")}, ensure_ascii=False)
        else:
            result = json.dumps({"status": "error", "message": res.get("error", "未知错误")}, ensure_ascii=False)
    elif tool_name == "load_skill_content":          # 新增分支
        skill_name = tool_args.get('skill_name', '')
        result = load_skill_content(skill_name)      # 直接返回技能正文文本
    else:
...
def main():
    ...
    # ------------------ 新增：动态构建技能列表并注入 system prompt ------------------
    skills = list_available_skills("agents/skills")
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "查询AnythingLLM工作区中的文档。当用户询问关于上传到AnythingLLM工作区中的文件内容时，使用此工具。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "question": {
                            "type": "string",
                            "description": "要向AnythingLLM工作区提问的问题"
                        }
                    },
                    "required": ["question"]
                }
        ...
        # ---------- 新增：技能加载工具 ----------
        {
            ...
            "function": {
                ...
                "description": "当需要使用某个技能时调用此函数，获取该技能的详细执行规则。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "skill_name": {
                            "type": "string",
                            "description": "技能名称，例如 'notice'"
                        }
                    },
                    "required": ["skill_name"]
                }
    ...
    # 系统提示词（融合技能列表）
    system_prompt = f"""你是一个具有文件操作、网络访问、聊天历史搜索、AnythingLLM文档查询以及动态技能调用能力的AI助手。

当前可用的技能列表如下：
{skills_json}

当用户的请求适合使用某个技能时，请先调用 load_skill_content 函数获取该技能的详细规则，然后严格按照技能规则执行。

其他可用工具：
1. 列出目录内容：使用 list_directory 工具
2. 重命名文件：使用 rename_file 工具
3. 删除文件：使用 delete_file 工具
4. 创建文件：使用 create_file 工具
5. 读取文件：使用 read_file 工具
6. 访问网页：使用 fetch_webpage 工具
7. 搜索聊天历史：使用 search_chat_history 工具
8. 查询AnythingLLM工作区文档：使用 query_anythingllm 工具

当用户输入以'/search'开头，或表达了'查找聊天历史'的意思时，请使用 search_chat_history 工具。
当用户询问关于上传文档的内容时，请使用 query_anythingllm 工具。
"""
    ...
    print("=== LLM 工具调用客户端（支持动态技能） ===")
    print("输入消息开始聊天，按 Ctrl+C 退出")
    print("=======================")
    print("可用工具：list_directory, rename_file, delete_file, create_file, read_file, fetch_webpage, search_chat_history, query_anythingllm, load_skill_content")
    print("已加载技能：", [s['name'] for s in skills])
    print()
    ...
    try:
        while True:
            ...
            if user_input.startswith('/search') or '查找聊天历史' in user_input or '搜索聊天' in user_input:
                ...
                # 检查是否需要总结聊天历史
                history_length = sum(len(str(msg.get('content', ''))) for msg in chat_history)
                ...
                if isinstance(response, dict):
                    ...
                    if message.get('tool_calls'):
                        # 添加助手消息（工具调用）到历史（只添加一次）
                        chat_history.append(message)
                        
                        # 标记是否需要额外注入技能规则 system 消息
                        skill_content_to_inject = None
                        ...
                        for tool_call in message.get('tool_calls', []):
                            ...
                            # 如果是加载技能，记录技能内容，稍后作为 system 消息注入
                            if tool_name == "load_skill_content":
                                # tool_result 已经是技能正文
                                skill_name = json.loads(tool_call.get('function', {}).get('arguments', '{}')).get('skill_name', '')
                                skill_content_to_inject = f"[技能 {skill_name} 的规则]\n{tool_result}"
                        
                        # 如果有技能规则需要注入，追加一条 system 消息
                        if skill_content_to_inject:
                            chat_history.append({
                                "role": "system",
                                "content": skill_content_to_inject
                            })
